...
- 修复开启双栏模式时左侧文件夹树小球尺寸被意外缩小的问题。
- 修复转动小球在轨道竖线上偏右的问题，并微调轨道基准彻底根除侧栏左边界裁切。
- 统一不可转动的人物素材尺寸与转动小球一致为 22px，居中且无裁切。
- 新增双栏小球、轨道居中与尺寸一致性测试，完整自动化测试通过 131/131。
...
- 修复双栏模式下更换小球样式时右栏未及时同步的问题，现在原生文件树与双栏浏览器会同时刷新。
- 优化样式切换反馈：小球在授权校验与设置保存完成前立即更新；授权失败时自动回退为足球小球。
- 新增双栏小球样式切换回归测试，完整自动测试通过 `125/125`。

## 0.4.0 — 2026-09-15

- 双栏新增准星定位、可选自动跟随当前文件、右键文件操作和当前目录新建。
- 侧边栏整体伸缩优化：拖拽侧边栏外侧分割线时主栏保持固定宽度，宽度增减全部自适应赋给二栏；主二栏中线分割线专门调整主栏像素宽度并持久化。
- 顶栏双栏切换按钮替换为定制分栏图标；双栏左侧返回单栏按钮替换为对称折叠图标。
- 键盘导航支持 Cmd+Enter / Ctrl+Enter 在新标签页打开文件；支持 F2 重命名与 Cmd+Backspace / Delete 触发原生安全删除确认。
- 子文件夹平铺汇总模式支持包含 `.canvas` 白板文件，并以专属看板图标呈现。
- 支持单文件跨栏移动、同名冲突保护及原生拖动描述对象；新增 Tab 跨栏导航与 F2 重命名。
- 新增左右/上下/自动布局，窄栏自动上下排列；窄文件栏顶部分两行，保留目录名称空间。
- 保留透明背景、Crisp 导轨、渐隐分割线和原生圆点工具栏，无新增运行依赖。

## 0.3.1 — 2026-09-15

- 双栏入口沿用原生工具栏尺寸，修复 Baseline 下展开时错位、收起时圆点被撑成竖条的问题。

- 新增可切换的轻量双栏：左侧独立文件夹导航和 Crisp 导轨，右侧为当前目录的子文件夹与文件列表。
- 延续主题透明底色；列表仅显示图标与名称，排序收进图标菜单，不读取正文或摘要。
- 支持返回上级、键盘导航、附件打开、Cmd/Ctrl 点击新标签页、分栏拖动及比例保存。整个侧栏宽度由用户控制。
- 原生虚拟文件树保留挂载，单栏继续使用原实现；双栏导航不改原生折叠状态。
- 重命名和删除时修正当前目录；普通工作区刷新保留列表 DOM、焦点和滚动位置。
